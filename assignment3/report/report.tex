...
\end{equation}
\begin{equation}
{\partial \hat{y_i} \over \partial W_{ij}} = {X_je^{(W_{ij}X_j+b_j)} \over C + e^{(W_{ij}X_j+b_j)} } - {X_je^{2(W_{ij}X_j+b_j)} \over (C + e^{(W_{ij}X_j+b_j)})^2} = X_j \hat{y_i} (1- \hat{y_i})
\end{equation}
Combining (6) and (8) to (4), and we will get the result:
\begin{equation}
{\partial l \over \partial \hat{y_i}} = {-y_i\over \hat{y_i}} = -X_jy_i(1- \hat{y_i})
\end{equation}
