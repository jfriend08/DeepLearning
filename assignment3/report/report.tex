...
\section{t-SNE}
\paragraph{(a)} Supose we have two-dimensional map that embedded with within a space with much higher dimensionality, and if in this high dimension each data points are mutually equaldistant, and the mapping is not ablt toe faithfully performed if the convergence is simply just based on mutual distance. Therefore, the crowding problem is defined as followed:
